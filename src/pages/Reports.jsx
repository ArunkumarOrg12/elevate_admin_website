...
export default function Reports() {
  ...
  const filtered =
    activeTab === "All"
      ? REPORT_TYPES
      : REPORT_TYPES.filter((r) => r.category === activeTab);
  ...
  return (
    <div className="page-enter space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">
            Institutional-grade PDF and Excel reports for decision makers
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {filtered.map((r) => (
          <Card key={r.id} className="p-5 hover-lift flex flex-col">
            <div className="flex items-start gap-3 mb-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-1">
                  <Badge variant={r.format === "PDF" ? "pdf" : "excel"}>
                    {r.format}
                  </Badge>
                  <span className="text-xs text-gray-400">{r.pages} pages</span>
                  <Badge
                    variant={r.status === "Ready" ? "completed" : "scheduled"}
                  >
                    {r.status}
                  </Badge>
                </div>
