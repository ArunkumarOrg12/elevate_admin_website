...
import {
  ...
  useRiskAlerts,
  useAcknowledgeAlert,
} from "../controllers/riskMonitorController";
...
export default function RiskMonitor() {
  ...
  const { data: alertsRes, isLoading: alertsLoading } = useRiskAlerts({
    limit: 200,
  });
  ...
  const stats = useMemo(() => {
    ...
    const criticalRisk = alerts.filter(
      (a) => a.risk_level === "critical",
    ).length;
    const decliningTrend = students.filter((s) => {
      const factors = Array.isArray(s.contributing_factors)
        ? s.contributing_factors
        : typeof s.contributing_factors === "string"
          ? s.contributing_factors.split(",").map((f) => f.trim())
          : [];
  ...
  const interventionCategories = useMemo(() => {
    const map = {};
    alerts.forEach((a) => {
      const factors = Array.isArray(a.contributing_factors)
        ? a.contributing_factors
        : typeof a.contributing_factors === "string"
          ? a.contributing_factors.split(",").map((f) => f.trim())
          : ["Unclassified"];
      factors.forEach((f) => {
        const label = f
          .replace(/_/g, " ")
    ...
    <div className="page-enter space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">
            {studentsLoading
              ? "Loading…"
              : `${stats.totalAtRisk} students require intervention · AY 2024-25`}
          </p>
        </div>
        ...
        <div className="rounded-[14px] border border-red-200 bg-red-50 p-4 flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-red-700 mt-0.5">
              {stats.totalAtRisk} students are below the employability
              threshold. Immediate intervention recommended for{" "}
              {criticalAlerts.length} critical-risk students
            ...
            <Button
              ...
              onClick={() =>
                criticalAlerts[0] &&
                acknowledgeAlert.mutate(criticalAlerts[0].id)
              }
              disabled={acknowledgeAlert.isPending}
            >
              {acknowledgeAlert.isPending
                ? "Acknowledging…"
                : "Acknowledge & View Action Plan"}
              <ArrowRight size={13} />
            </Button>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          description="EI < 40 · Urgent"
          icon={AlertOctagon}
          accentColor="red"
        />
        <StatCard
          label="DECLINING TREND"
          value={stats.decliningTrend}
          description="Negative velocity"
          icon={TrendingDown}
          accentColor="amber"
        />
        <StatCard
          label="INTERVENTION PENDING"
          value={stats.interventionPending}
                ...
                <ResponsiveContainer width="100%" height={160}>
                  <PieChart>
                    <Pie
                      ...
                      dataKey="value"
                    >
                      {interventionCategories.map((e, i) => (
          ...
          <Table>
            ...
            <TableBody>
              {students.map((s) => {
                ...
                const factors =
                  (Array.isArray(s.contributing_factors)
                    ? s.contributing_factors
                    : typeof s.contributing_factors === "string"
                      ? s.contributing_factors.split(",").map((f) => f.trim())
                      : []
                ...
                return (
                  <TableRow key={s.id}>
                    ...
                    <TableCell>
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full font-medium ${URGENCY_VARIANTS[urgency] ?? URGENCY_VARIANTS.medium}`}
                      >
        ...
        {pages > 1 && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100 bg-gray-50">
            <p className="text-xs text-gray-500">
              ...
              {pagination.total === 0
                ? 0
                : Math.min((page - 1) * PER_PAGE + 1, pagination.total)}
              –{Math.min(page * PER_PAGE, pagination.total)} of{" "}
