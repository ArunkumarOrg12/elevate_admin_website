  ...
  return (
    <div>
      ...
      <input type={type} defaultValue={value}
        className="w-full border border-gray-200 rounded-[9px] px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
    </div>
  );
}
          ...
          <div className="card p-2 flex md:flex-col gap-1 overflow-x-auto">
            {tabs.map(t => (
              <button key={t.id} onClick={() => setActiveTab(t.id)}
                className={`whitespace-nowrap md:w-full text-left px-3 py-2.5 rounded-[9px] text-sm font-medium transition-colors ${activeTab === t.id ? 'bg-indigo-600 text-white' : 'text-gray-700 hover:bg-gray-100'}`}>
                {t.label}
              </button>
            ))}
          </div>
        ...
        <div className="flex-1 space-y-4">
          {activeTab === 'general' && (
            <>
              <div className="card p-5">
                ...
                <button className="btn-primary flex items-center gap-1.5 text-sm mt-4"><Save size={14} /> Save Changes</button>
              </div>
              <div className="card p-5">
                ...
                <button className="btn-primary flex items-center gap-1.5 text-sm mt-4"><Save size={14} /> Save</button>
              </div>
            </>
          )}
          ...
          {activeTab === 'notifications' && (
            <div className="card p-5">
              ...
              <button className="btn-primary flex items-center gap-1.5 text-sm mt-4"><Save size={14} /> Save Preferences</button>
            </div>
          )}
            ...
            <div className="card p-5">
              ...
              <div className="space-y-4">
                <div className="p-4 bg-emerald-50 rounded-xl border border-emerald-200">
                  ...
                  <input type="number" defaultValue={EI_THRESHOLDS.ready} min={0} max={100}
                    className="w-32 border border-emerald-300 rounded-[9px] px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500" />
                ...
                <div className="p-4 bg-red-50 rounded-xl border border-red-200">
                  ...
                  <input type="number" defaultValue={EI_THRESHOLDS.developing} min={0} max={100}
                    className="w-32 border border-red-300 rounded-[9px] px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500" />
                  <p className="text-xs text-red-600 mt-1">Students with EI &lt; this score are classified as At Risk</p>
                </div>
              </div>
              <button className="btn-primary flex items-center gap-1.5 text-sm mt-4"><Save size={14} /> Update Thresholds</button>
            </div>
          )}
                ...
                {['Dr. V. Anand — Presidency EC', 'Dr. Ramesh Babu — PSG CoT', 'Prof. Suresh Kumar — SSN CE'].map(u => (
                  <div key={u} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button className="text-xs text-red-600 border border-red-200 rounded-[7px] px-2 py-1 hover:bg-red-50">Disable</button>
                    </div>
                  </div>
                ))}
              </div>
              <button className="btn-primary text-sm mt-4">+ Add College Admin</button>
            </div>
          )}
            ...
            <div className="card p-5">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input type="number" defaultValue={30} className="w-32 border border-gray-200 rounded-[9px] px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Minimum Password Length</label>
                  <input type="number" defaultValue={8} className="w-32 border border-gray-200 rounded-[9px] px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                </div>
              ...
              <button className="btn-primary flex items-center gap-1.5 text-sm mt-4"><Save size={14} /> Save Security Settings</button>
            </div>
          )}
        </div>
      </div>
