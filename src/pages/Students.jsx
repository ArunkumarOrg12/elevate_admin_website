...
export default function Students() {
  ...
  const counts = {
    ...
    industryReady: students.filter((s) => s.riskCategory === "INDUSTRY_READY")
      .length,
    placementReady: students.filter((s) => s.riskCategory === "PLACEMENT_READY")
      .length,
  ...
  if (isLoading)
    return <div className="p-8 text-gray-500">Loading students…</div>;
  if (isError)
    return (
        ...
        <div className="flex gap-2 flex-shrink-0">
          ...
          <RoleGuard allowedRoles={[ROLES.SUPER_ADMIN, ROLES.COLLEGE_ADMIN]}>
            <Button
              variant="secondary"
              size="sm"
              onClick={() => handleDownloadTemplate(user?.college_id)}
            >
      ...
      <Card className="overflow-hidden">
            ...
            {paged.length === 0 ? (
              ...
              paged.map((s) => (
                <TableRow key={s.id} className="hover:bg-gray-50">
                  ...
                  <TableCell>
                    <div
                      className={`text-sm font-semibold ${getEIColor(s.eiScore)}`}
                    >
                      {s.eiScore}
                    </div>
                    <div className="w-16 h-1.5 bg-gray-100 rounded-full mt-1 overflow-hidden">
                      <div
                        className={`h-full rounded-full ${getEIBgColor(s.eiScore)}`}
                        style={{ width: `${s.eiScore}%` }}
                      />
                    </div>
                  </TableCell>
                  ...
                  <TableCell>
                    {s.velocity === null ? (
                      ...
                      <span
                        className={`text-sm font-medium ${s.velocity >= 0 ? "text-emerald-600" : "text-red-600"}`}
                      >
                        {s.velocity >= 0 ? "+" : ""}
                        {s.velocity} pts
                      </span>
                  ...
                  <TableCell>
                    <StatusBadge riskCategory={s.riskCategory} />
                  </TableCell>

                  <TableCell className="text-sm text-gray-500">
                    {formatDate(s.lastAssessment)}
                  </TableCell>
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100 bg-gray-50">
          <p className="text-xs text-gray-500">
            Showing{" "}
            {total === 0 ? 0 : Math.min((page - 1) * PER_PAGE + 1, total)}–
