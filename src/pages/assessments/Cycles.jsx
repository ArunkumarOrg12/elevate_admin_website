    ...
    createMutation.mutate(
      {
        ...
        ...(form.minutes_per_question && {
          minutes_per_question: Number(form.minutes_per_question),
        }),
        ...(form.topics_covered.trim() && {
          topics_covered: parseLines(form.topics_covered),
        }),
        ...
        <div className="px-6 py-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              ...
              <Select
                ...
                onValueChange={(val) =>
                  setForm((p) => ({ ...p, paper_set: val }))
                }
                disabled={psLoading}
              >
                <SelectTrigger>
                  <SelectValue
                    placeholder={psLoading ? "Loading..." : "Select"}
                  />
                </SelectTrigger>
                <SelectContent>
                  {paperSets.map((ps, i) => {
                    const id =
                      typeof ps === "string"
                        ? ps
                        : String(ps.id || ps._id || i);
                    const label =
                      typeof ps === "string"
                        ? ps
                        : ps.name || ps.title || ps.setId || id;
                    return (
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <Popover
                open={calOpen.start}
                onOpenChange={(v) => setCalOpen((p) => ({ ...p, start: v }))}
              >
                <PopoverTrigger asChild>
                  <button
                    className={`flex h-9 w-full items-center gap-2 rounded-[9px] border border-gray-200 bg-white px-3 py-1.5 text-sm ${form.start_date ? "text-gray-900" : "text-gray-400"}`}
                  >
                ...
                <PopoverContent align="start" className="w-auto p-0">
                  <CalendarComponent
                    ...
                    onSelect={(d) => {
                      setForm((p) => ({ ...p, start_date: d ?? null }));
                      setCalOpen((p) => ({ ...p, start: false }));
                    }}
                    initialFocus
                  />
                </PopoverContent>
            ...
            <div className="space-y-1.5">
              ...
              <Popover
                open={calOpen.end}
                onOpenChange={(v) => setCalOpen((p) => ({ ...p, end: v }))}
              >
                <PopoverTrigger asChild>
                  <button
                    className={`flex h-9 w-full items-center gap-2 rounded-[9px] border border-gray-200 bg-white px-3 py-1.5 text-sm ${form.end_date ? "text-gray-900" : "text-gray-400"}`}
                  >
                    <Calendar size={14} className="text-gray-400" />
                    {form.end_date
                      ? format(form.end_date, "dd MMM yyyy")
                      : "Pick date"}
                  </button>
                </PopoverTrigger>
                <PopoverContent align="start" className="w-auto p-0">
                  <CalendarComponent
                    ...
                    onSelect={(d) => {
                      setForm((p) => ({ ...p, end_date: d ?? null }));
                      setCalOpen((p) => ({ ...p, end: false }));
                    }}
                    initialFocus
                  />
                </PopoverContent>
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              onChange={(e) =>
                setForm((p) => ({ ...p, minutes_per_question: e.target.value }))
              }
            />
          </div>
          <div className="space-y-1.5">
            ...
            <textarea
              ...
              onChange={(e) =>
                setForm((p) => ({ ...p, topics_covered: e.target.value }))
              }
              className="w-full rounded-[9px] border border-gray-200 px-3 py-2 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-colors"
            />
          </div>
...
function CycleResultsDialog({ cycleId, cycleName, open, onOpenChange }) {
  const { data: resultsData, isLoading: rLoading } = useCycleResults(
    open ? cycleId : null,
  );
  const { data: lbData, isLoading: lbLoading } = useCycleLeaderboard(
    open ? cycleId : null,
  );
            ...
            {rLoading ? (
              ...
              <div className="space-y-1.5">
                {results.slice(0, 5).map((r, i) => (
                  <div
                    key={i}
                    className="flex items-center justify-between text-sm bg-gray-50 px-3 py-2 rounded-[8px]"
                  >
                    <span className="text-gray-700">
                      {r.studentName || r.name || `Student ${i + 1}`}
                    </span>
  ...
  const filtered = useMemo(() => {
    return cycles.filter((c) => {
      const matchSearch =
        ...
        c.batch?.toLowerCase().includes(searchQ.toLowerCase());
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">
            Manage assessment cycles and view results
          </p>
        </div>
        <RoleGuard
          allowedRoles={[
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              ...
              <CardDescription>
                {isLoading
                  ? "Loading..."
                  : `${filtered.length} of ${cycles.length} cycle${cycles.length !== 1 ? "s" : ""}`}
              </CardDescription>
            </div>
            {/* Filters */}
                    ...
                    <TableRow key={cId}>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-1.5 text-xs text-gray-600">
                          ...
                          <span>
                            {cycle.start_date
                              ? format(new Date(cycle.start_date), "dd MMM yy")
                              : "—"}
                            {cycle.end_date
                              ? ` → ${format(new Date(cycle.end_date), "dd MMM yy")}`
                              : ""}
                          </span>
                        </div>
                      </TableCell>
                          ...
                          <div className="space-y-0.5">
                            <div className="flex items-center gap-1 text-sm text-gray-700">
                              ...
                              <span className="font-medium">
                                {cycle.participation.total_started}
                              </span>
                            ...
                            <div className="flex items-center gap-2 text-xs text-gray-400 pl-4">
                              <span className="text-emerald-600">
                                {cycle.participation.total_submitted} submitted
                              </span>
                              {cycle.participation.in_progress > 0 && (
                                <span className="text-blue-500">
                                  {cycle.participation.in_progress} in progress
                                </span>
                              )}
                            </div>
                          </div>
                      ...
                      <TableCell>
                        <span
                          className={`text-xs px-2 py-0.5 rounded-full font-medium capitalize ${STATUS_COLORS[cycle.status] || "bg-gray-100 text-gray-600"}`}
                        >
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          >
                            <Button
                              ...
                              onClick={() =>
                                navigate(
                                  `/assessments/cycles/${cId}/participants`,
                                )
                              }
