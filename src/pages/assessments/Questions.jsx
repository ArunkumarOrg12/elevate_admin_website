...
import {
  Plus, Search, Trash2, Eye, Edit2, HelpCircle, ImageIcon, AlertCircle,
} from 'lucide-react';
...
import { useQuestions, useDeleteQuestion } from '../../controllers/questionsController';
    ...
    <div className="page-enter space-y-5">
      ...
      <Dialog open={!!deleteTarget} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <DialogContent className="max-w-sm">
          ...
          <p className="text-sm text-gray-600 px-6 pb-2">
            Are you sure you want to delete this question? It will be removed from all paper sets.
          </p>
          <DialogFooter>
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">Manage assessment question bank</p>
        </div>
        <Button size="sm" onClick={() => navigate('/assessments/questions/add')}>
      ...
      <Card className="overflow-hidden">
        ...
        {isError ? (
          ...
          <div className="px-5 py-10 text-center text-sm text-gray-400">Loading questions...</div>
        ) : questions.length === 0 ? (
          <div className="flex flex-col items-center gap-3 py-14 text-gray-400">
            <HelpCircle size={32} strokeWidth={1.5} />
            <p className="text-sm">No questions found. <button className="text-indigo-600 hover:underline" onClick={() => navigate('/assessments/questions/add')}>Add one</button></p>
          </div>
        ) : (
          <Table>
            <TableHeader>
              <TableRow>
                {['#', 'QUESTION', 'TYPE', 'DIFFICULTY', 'MARKS', 'TAGS', 'ACTIONS'].map(h => (
                ...
                <TableRow key={q.id}>
                  ...
                  <TableCell className="max-w-[280px]">
                    <div className="flex items-start gap-2">
                      ...
                      <span className="text-sm text-gray-800 line-clamp-2">{questionText}</span>
                    </div>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell className="text-sm text-gray-700">{marks ?? '—'}</TableCell>
                  <TableCell>
                    {tags.length > 0 ? (
                  ...
                  <TableCell>
                    <div className="flex items-center gap-1">
                      <Button
                        ...
                        onClick={() => navigate(`/assessments/questions/${q.id}`)}
                        title="View / Edit"
                      >
                        <Eye size={14} />
                      </Button>
